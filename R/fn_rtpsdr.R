...
#' {
#'   p <- ncol(x) #new data
#'   if(ncol(as.matrix(x)) != p)
#'     stop("dimension of the new data must be matched to the previouse data.")
#'
#'   n <- obj$N #old data
#'   m <- nrow(x) #new data
#'
#'   #new update quantities
#'   #new A matrix
#'   tmp.rslt <- A.rtpsdr(x, y, obj, h, lambda)
#'   A.new <- A <- tmp.rslt$A
#'   Xbar <- tmp.rslt$Xbar
#'   #new coefficient
#'   r.new <- r  <- coefficient.rtpsdr(x, y, obj, h=10, lambda=1)$r
#'
#'
#'   Mn <- t(r.new[,1:p])%*%r.new[,1:p]
#'   eigen.Mn <- eigen(Mn)
#'   tmp <- list("Mn"=Mn, "evalues" = eigen.Mn$values, "evectors" = eigen.Mn$vectors,
#'               r=r.new, A=A, "x" = x, "y" = y, N=n+m, Xbar=Xbar )
#'   class(tmp) <- "psdr"
#'   return(tmp)
#' }
#'
#'
#' #' @noRd
#' A.rtpsdr <- function(x, y, obj, h=10, lambda=1){
#'   x <- as.matrix(x)
#'   p <- ncol(x)
#'   H <- h
#'
...
#'
#'   new.Xbar <- apply(x, 2, mean)
#'   Xbar <- (new.Xbar*m+Xbar*n)/(n+m) #new data
#'   x <- cbind(t(t(x)-Xbar), -1)
#'   cov.new.X <- cov(x)*(m-1)/m
#'
#'   if(sum(unique(y)) != 0){#plssvm
...
#'     }
#'   }
#'   newlist <- list(A=A.new, Xbar=Xbar)
#'   class(newlist) <- "psdr"
#'   return(newlist)
#' }
#'
#' #' @noRd
#' coefficient.rtpsdr <- function(x, y, obj, h=10, lambda=1){
#'   H <- h
#'   x <- as.matrix(x)
#'   qprob <- (1:(h-1))/h
#'   qy <- stats::quantile(y, qprob)
...
#'       s_part <- diag(rep(1, p+1))+A[[i]]%*%B_new
#'       s <- diag(rep(1, p+1))-A[[i]] %*% B_new %*% solve(s_part)
#'       r.new[i,] <- t(s %*% (r.new[i,] + A[[i]] %*% C_new))
#'     }
#'   }
#'   newlist <- list(r = r.new)
#'   class(newlist) <- "psdr"
#'   return(newlist)
#' }
#'
