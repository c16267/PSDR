                  ...
                  eps=1.0e-5, max.iter=100, eta=0.1, plot=TRUE)
{
  if(sum(as.character(loss) == c("lssvm", "wlssvm")) == 0){
    ...
    if(is.null(eta) == T ){
      writeLines("eta is a learning rate which should be specified as a positve value. It is set to 0.1")
      eta <- 0.1
    }
    if(is.null(loss)==T){
    ...
    if(is.null(max.iter) == T){
      writeLines("max.iter is set to 100 as a default.")
      max.iter <- 100
    }
  }
  if(is.null(lambda) == T){
