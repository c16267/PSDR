#'Determination of structured dimension
#' @description
#'Estimation of a structural dimensionality. Choose the k which maximizes a Gn value
#'@param obj The gdsdr object
#'@param x data matrix X
#'@param rho hyperparameter of CVBIC
#'@param plot if plot=TRUE, plot the estimated Gn values
#'@param ... generic attributes
#'@return Estimated Gn scores for a optimal structural dimension
#'@author Jungmin Shin, \email{jungminshin@korea.ac.kr}, Seungjun Shin, \email{sjshin@korea.ac.kr}
#'@seealso \code{\link{GD.psdr.solver}} \code{\link{plot.psdr}}
#'@examples
#'\donttest{
#'set.seed(1234)
#'n <- 200; p <- 5; H <- 10; lambda <- 0.001; eps <- 1.0e-6;
#'max.iter <- 10; h <- 1.0e-5; delta <- 2*1.0e-1;
#'init.theta <- rnorm(sd=1,n=p)
#'x <- matrix(rnorm(n*p, 0, 1), n, p)
#'err <- rnorm(n, 0, .2)
#'B <- matrix(0, p, 2)
#'B[1,1] <- 1; B[2,2] <- 1
#'x1 <- x %*% B[,1]
#'x2 <- x %*% B[,2]
#'fx <-  x1/(0.5 + (x2 + 1)^2)
#'y <- fx + err
#'obj <- GD.psdr.solver(x, y, init.theta, H,lambda, h, delta, eps, max.iter, loss="svm")
#'Gn(obj, x, rho=0.005, plot=TRUE) }
#'@import stats graphics
#'@export Gn


#Gn <- function(x, ...){
#  UseMethod("Gn")
#}


Gn <- function(obj, x, rho, plot=FALSE, ...) {
  p <- nrow(obj$vectors)
  v <- obj$values
  n <- nrow(x)
  temp_Mat <- diag(p); temp_Mat[lower.tri(temp_Mat)] <- 1
  temp_Mat
  temp_vec <- c(1:p)
  value <- temp_Mat %*% v - temp_vec * (rho * log(n) / n^(1/2) * v[1])
  if(plot == TRUE){
    x.vec <- seq(1, length(value), by=1)
    plot <- plot(x.vec, value, type = "b", xlab=expression(m), ylab=expression(G[n](m)))
    plot
  }
  class(value) <- "psdr"
  return(value)
}


